...
void UnityAssertEqualUintNumber(const UNITY_UINT expected, const UNITY_UINT actual,
                                const char *msg, const UNITY_LINE_TYPE lineNumber,
                                const UNITY_DISPLAY_STYLE_T style) {
    RETURN_IF_FAIL_OR_IGNORE;
...
void UnityAssertUintGreaterOrLessOrEqualNumber(
    const UNITY_UINT threshold, const UNITY_UINT actual, const UNITY_COMPARISON_T compare,
    ...
    while ((elements > 0) && (elements--)) {
        ...
        if (expect_val != actual_val) {
            if ((style & UNITY_DISPLAY_RANGE_UINT)
                && (length
                    < (UNITY_INT_WIDTH / 8))) { /* For UINT, remove sign extension (padding 1's)
