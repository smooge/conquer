...
GCC_BASE_FLAGS="-std=c2x -D_POSIX_C_SOURCE=200809L"
GCC_OPTIMIZATION_FLAGS="-O2 -g"
...
WARNING_LEVELS[10]="-Wall -Wextra -Wpedantic -Wformat=2 -Wconversion -Wsign-conversion -Wimplicit-fallthrough -Wstrict-prototypes -Wold-style-declaration -Wshadow -Wmissing-prototypes -Wcast-qual -fanalyzer"

# File compilation flags (based on test_warnings.sh file categorization)
ADMIN_FLAGS="${GCC_OPTIMIZATION_FLAGS} ${GCC_BASE_FLAGS} ${DFLAGS} -DADMIN -DCONQUER"
GAME_FLAGS="${GCC_OPTIMIZATION_FLAGS} ${GCC_BASE_FLAGS} ${DFLAGS} -DCONQUER"
PSMAP_FLAGS="${GCC_OPTIMIZATION_FLAGS} ${GCC_BASE_FLAGS} ${DFLAGS} -DPSFILE=\"/projects/conquer/bin/psmap.ps\" -DLETTER"
...
show_help() {
    cat << EOF
${SCRIPT_NAME} v${SCRIPT_VERSION} - GCC Compilation Testing Script
...
EXAMPLES:
    ...
    ./${SCRIPT_NAME} -w 10              # Maximum warning level with analyzer
...
- Optimization: \`${GCC_OPTIMIZATION_FLAGS}\`
...
test_gcc_compilation() {
    log_message "INFO" "Starting GCC compilation test with file-type detection"
    ...
    log_message "INFO" "Testing CMake build system with GCC"
    echo "### CMake Build System Test" >> "$REPORT_FILE"
    local cmake_warnings=0
    local cmake_output=""

    if cmake_output=$(cmake --build build --clean-first 2>&1); then
        cmake_warnings=$(echo "$cmake_output" | grep -c "warning:" || true)
        log_message "SUCCESS" "CMake build successful - $cmake_warnings warnings"
        echo "**Status**: ✅ PASSED - $cmake_warnings warnings" >> "$REPORT_FILE"
        total_warnings=$((total_warnings + cmake_warnings))
    else
        log_message "ERROR" "CMake build failed"
        echo "**Status**: ❌ FAILED" >> "$REPORT_FILE"
        failed_files=$((failed_files + 1))
    fi
    echo "**Command**: \`cmake --build build --clean-first\`" >> "$REPORT_FILE"
    echo "" >> "$REPORT_FILE"
...
EOF
    ...
    if [[ $failed_files -eq 0 ]]; then
        log_message "SUCCESS" "All GCC tests passed - $success_tests/$total_tests tests, $total_warnings total warnings"
        return 0
    else
        log_message "ERROR" "$failed_files GCC tests failed - $success_tests/$total_tests passed"
        return 1
    fi
}
...
main() {
    ...
    log_message "INFO" "Starting GCC compilation testing"
    init_report
