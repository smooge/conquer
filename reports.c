...
/* 0 is for full report 'a' and 1 is for group report 'g' */
{
    ...
    while (done == FALSE) {
        ...
        ypos++;
        if ((inpkey = safe_int_to_char(getch())) == ' ')
            done = TRUE;
        else if ((inpkey == '\n') || (inpkey == '\r')) {
            done = 0;
            ...
            if (isgod == TRUE)
                mvaddstr(ypos++, 0,
                         "GOD OPTIONS: 6) LOCATION 7) SOLDIERS 8) MOVE 9) UNITTYPE 0) STATUS ");
            clrtoeol();
#endif /* OGOD */
            ypos++;
            refresh();
            switch (getch()) {
                ...
                case '0':
                    if (isgod == TRUE) {
                        mvprintw(ypos++, 0,
                                 "1) Mar 2) Sct 3) Garr 4) Trade 5) Mil 6) Fly 7) Def 8) MDef "
                                 "9) Att 10) MAtt");
                        mvprintw(ypos++, 0,
                                 "11) Genrl 12) Sort 13) Sieg 14) Sgd 15) Onb 16) Rule 17+) "
                                 "Group (leader-=17)");
            ...
            while (armynum < MAXARM
                   && (P_ASOLD == 0 || !(!repnum || (P_AXLOC == XREAL && P_AYLOC == YREAL))))
                armynum++;
            if (armynum == MAXARM)
                count2 = 0;
            else
...
void budget(void) {
    ...
    mvprintw(12, COLS - 50, "%8ld ship holds @ %4ld....%8ld", numship, SHIPMAINT,
             numship * SHIPMAINT);
...
#pragma GCC diagnostic ignored "-Wconversion"
    mvprintw(15, 0, "ESTIMATE NET FOOD.........%8.0f tons",
             spread.food - curntn->tfood - P_EATRATE * safe_long_to_double(military));
#pragma GCC diagnostic pop
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wconversion"
    mvprintw(16, 0, "ESTIMATE FOOD SUPPLY......%8.0f tons",
             spread.food - P_EATRATE * safe_long_to_double(military));
#pragma GCC diagnostic pop
    standend();
    fprintf(fp, "LINE %d FILE %s\n", __LINE__, __FILE__);
    ...
    while (done == FALSE) {
        ...
        while ((nvynum < MAXNAVY) && (count < MAXINSCR)) {
            if ((P_NWSHP != 0) || (P_NMSHP != 0) || (P_NGSHP != 0)) {
                ...
                mvprintw(ypos + 2, xpos, "%2hd/%2hd/%2hd", P_NWAR(N_LIGHT), P_NWAR(N_MEDIUM),
                         P_NWAR(N_HEAVY));
                mvprintw(ypos + 3, xpos, "%2hd/%2hd/%2hd", P_NMER(N_LIGHT), P_NMER(N_MEDIUM),
                         P_NMER(N_HEAVY));
                mvprintw(ypos + 4, xpos, "%2hd/%2hd/%2hd", P_NGAL(N_LIGHT), P_NGAL(N_MEDIUM),
                         P_NGAL(N_HEAVY));
        ...
        if ((navy = safe_int_to_short(getch())) == ' ')
            done = TRUE;
        else if ((navy == '\n') || (navy == '\r')) {
            ...
            switch (getch()) {
                    ...
                    if (curntn->nvy[newnavy].commodity == TRADED) {
                        ...
                        if (nvynum == newnavy) {
                            ...
                            for (i = N_LIGHT; i <= N_HEAVY; i++) {
                                if (P_NGAL(i) + SHIPS(curntn->nvy[newnavy].galleys, i)
                                    > N_MASK) {
                                    i = (-1);
                                    break;
                                }
                            ...
                            for (i = N_LIGHT; i <= N_HEAVY; i++) {
                                (void)addwships(newnavy, safe_int_to_short(i),
                                                safe_int_to_short(P_NWAR(i)));
                ...
                case '4':
                    if (isgod == TRUE) {
                        ...
                        for (shipsize = N_LIGHT; shipsize <= N_HEAVY; shipsize++) {
                            mvprintw(ypos, 0, "How Many %s Merchants? [%d] ", fltstr[shipsize],
                                     P_NMER(shipsize));
                            clrtoeol();
                            refresh();
                            newnavy = safe_long_to_short(get_number());
            ...
            while ((nvynum < MAXNAVY) && ((P_NWSHP == 0) && (P_NMSHP == 0) && (P_NGSHP == 0)))
                nvynum++;
            if (nvynum == MAXNAVY)
                count2 = 0;
            else
