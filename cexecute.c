...
int execute(int isupdate) { /* 0 if not update, 1 if update */
    ...
    for (x = 0; x < MAPX; x++)
        for (y = 0; y < MAPY; y++)
            if ((sct[x][y].owner == country)
                && ((sct[x][y].designation == DTOWN) || (sct[x][y].designation == DCAPITOL)
    ...
    while (done == FALSE) {
        ...
        if (line[0] == 'L' && line[1] == '_') {
            int result = sscanf(line, "%79s %d %hd %ld %ld %hd %79s", temp, &cmd, &country,
                                &longvar, &long2var, &y, comment);
            if (result != 7) {
            ...
            int result = sscanf(line, "%79s %d %hd %d %hd %hd %79s", temp, &cmd, &country,
                                &armynum, &x, &y, comment);
            if (result != 7) {
                /* Parse error - skip malformed command line */
                temp[0] = '\0';
        ...
        switch (cmd) {
            ...
            case MSETB: /*Aadjdisb*/
                ...
                if (x > MERCATT)
                    MERCATT = safe_long_to_short((MERCMEN * MERCATT + armynum * x)
                                                 / (MERCMEN + armynum));
                if (y > MERCDEF)
                    MERCDEF = safe_long_to_short((MERCMEN * MERCDEF + armynum * y)
                                                 / (MERCMEN + armynum));
                MERCMEN += armynum;
                break;
            case XNLOC: /*nadjloc*/
            ...
            case XSAOWN:
                ...
                if ((sct[x][y].owner != country) && (country != 0) && (sct[x][y].owner != 0)
                    && (sct[x][y].people > 0) && (magic(country, SLAVER) == FALSE)
                    && (ntn[sct[x][y].owner].race != curntn->race)) {
                    sct[ntn[sct[x][y].owner].capx][ntn[sct[x][y].owner].capy].people +=
                        sct[x][y].people;
                    sct[x][y].people = 0;
                    fprintf(stderr,
                            "ERROR: <%s> taking sector %d %d but civilians exist of other race "
                            "- puting them in their capitol\n",
                            curntn->name, x, y);
                }
            ...
            case CHG_MGK:
                ...
                if (curntn->powers != longvar) {
                    printf("\nERROR ON MAGIC READ country=%d %ld != %ld (or of %ld)", country,
                           longvar, curntn->powers, long2var);
                    getchar();
                }
