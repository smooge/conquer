...
static char *Classwho[] = {"DEHO", "DEH", "DEH", "EH", "H", "DHO", "HE", "DHO", "O", "O", "O"};
...
static char *Mlabels[] = {"Population",   "Treasury",      "Location",     "Military",
...
static char *Mitems[] = {"people",  "gold talons",    "location",     "soldiers",
                         "percent", "percent",        "percent",      "move points",
                         "powers",  "nation leaders", "units of food"};
 ...
 *   Key Tests: [Terrain modification per race, resource placement probability, range
 * boundaries] Dependencies: [Global world array sct[][], race constants, resource functions]
 ...
 *   Key Tests: [Character found, character not found, empty string, single character, NULL
 * handling] Dependencies: [strlen() function from string.h, TRUE/FALSE constants] Mock
 * Requirements: None - pure function with standard library dependencies Complexity: Simple -
 * straightforward linear search algorithm
 *
 * Notes:
 ...
 *   Key Tests: [Location formatting, numeric formatting, raw materials calculation, period
 * termination] Dependencies: [Curses library, LType[] array, Mitems[] array,
 * NLJEWELS/NLMETAL/Mvalues constants] Mock Requirements: [Terminal screen, global item arrays,
 * calculation constants] Complexity: Moderate - conditional formatting with calculation logic
 *
 * Notes:
 ...
 *   Dependencies: [Curses library, spent[] array, LType/Mitems arrays, Mvalues constants,
 * NLJEWELS/NLMETAL] Mock Requirements: [Terminal screen, global allocation arrays, calculation
 * constants] Complexity: Moderate - conditional formatting with preprocessor compilation paths
 *
 * Notes:
...
void showitem(int line, int item) {
    ...
    /* now show the extras for the Raw Materials */
#if NLJEWELS == NLMETAL
    snprintf(tempc, sizeof(tempc), "%ld jewels & metal", spent[CH_RAWGOODS] * NLJEWELS);
    mvprintw(line, 0, "%38s", tempc);
    mvprintw(line, COLS / 2 + 13, "%ld jewels & metal", NLJEWELS);
#else
    snprintf(tempc, sizeof(tempc), "%ld jewels", spent[CH_RAWGOODS] * NLJEWELS);
    mvprintw(line, 0, "%38s", tempc);
    mvprintw(line++, COLS / 2 + 10, "%ld jewels", NLJEWELS);
    snprintf(tempc, sizeof(tempc), "%ld metal", spent[CH_RAWGOODS] * NLMETAL);
    mvprintw(line, 0, "%38s", tempc);
 ...
 *   Key Tests: [Multiplier calculations, location conversion, magic acquisition, resource
 * allocation] Dependencies: [Global spent[] array, curntn nation pointer, Mvalues[] constants,
 * magic system] Mock Requirements: [Nation structure, magic functions, random number generator]
...
void convert(void) {
    ...
    curntn->maxmove = safe_long_to_uchar(Mvalues[CH_MOVEMENT] * spent[CH_MOVEMENT]);
 ...
 *
 * Testing Notes:
 *   Category: C (System) - Requires full game environment with curses, file system, magic
 * system Approach: System testing with complete game world setup and user interaction
 * simulation Key Tests: [Complete registration workflow, race-specific features, point
 * allocation system, input validation, error handling, multi-nation registration, curses
 * interface] Dependencies: [Curses library, nation arrays, magic system, map system, file I/O,
 *                 password encryption, mail system, global game state, constants/arrays]
 *   Mock Requirements: [Complete game world, terminal interface, file system, user input
 * simulation, random number generation, cryptographic functions, mail delivery system]
...
void newlogin(int realuser) {
...
#ifdef CHECKUSER
            if ((ntn[i].uid == realuser) && (ntn[0].uid != realuser)) {
    ...
    while (more == TRUE) {
        ...
        while (valid == FALSE) {
            valid = TRUE;
            mvaddstr(2, 0,
                     "Enter the name of your country's leader (Ex. The Ed, Gandalf, Conan)");
            clrtoeol();
        ...
        standout();
        mvaddstr(LINES - 4, 2,
                 "DONE=ESC  EXEC=SPACE  INFO=\"?\"  ADD=\"<+h\"  SUBtract=\">+l\"  UP=\"k\"  "
                 "DOWN=\"j\"");
        standend();
 ...
 *   - Water tolerance varies by world water percentage (>50%: 7+ water rejected, ≤50%: 5+ water
 * rejected)
 ...
 *   - Territory acquisition requires: sufficient food production, habitable, unowned,
 * unpopulated
 ...
 *   Key Tests: [Placement success rates, territory allocation, army distribution, failure
 * recovery] Dependencies: [Global game state, map data, terrain functions, curses display,
 * nation arrays] Mock Requirements: [Complete game world with various terrain types and
 * existing nations] Complexity: Complex - Full system integration with multiple subsystems and
 * failure handling
 *
 * Notes:
 ...
 */
void place(int xloc, int yloc /* if not -1,-1 should place in this spot */
) {
        ...
        case RANDOM:
            while ((placed == 0) && (n++ < 2000)) {
                if (ispc(curntn->active)) {
                    ...
                    for (i = x - 2; i <= x + 2; i++)
                        for (j = y - 2; j <= y + 2; j++)
                            if ((isntn(ntn[sct[i][j].owner].active)) && (sct[i][j].owner != 0))
                                placed = 0;
                } else {
                    x = (rand() % (MAPX - 6)) + 3;
                    y = (rand() % (MAPY - 6)) + 3;
    ...
    if (placed == 1) {
        ...
        sct[x][y].tradegood =
            safe_int_to_uchar(rand() % (END_KNOWLEDGE - END_SPOILRATE) + END_SPOILRATE + 1);
        sct[x][y].jewels = 0;
        sct[x][y].metal = 0;
        ...
        for (i = x - t; i <= x + t; i++)
            for (j = y - t; j <= y + t; j++)
                if ((tofood(&sct[i][j], country) >= DESFOOD) && (sct[i][j].owner == 0)
                    && (is_habitable(i, j) == TRUE) && (sct[i][j].people == 0)) {
                    curntn->tsctrs++;
 ...
 *   Key Tests: [Race eligibility filtering, special pricing, input validation, display
 * formatting] Dependencies: [Curses display system, class configuration arrays, doclass()
 * function] Mock Requirements: [Curses environment, class data arrays, user input simulation]
 ...
 *   Key Tests: [Class filtering by race, cost calculation with modifiers, input validation,
 * menu display, screen cleanup] Dependencies: [Class arrays (Class[], Classwho[], CPowlist[],
 * Classcost[]), ncurses, doclass(), in_str(), newerror()] Mock Requirements: [Class
 * configuration data, nation structure, ncurses functions, global constants] Complexity:
 * Moderate - complex UI with validation logic and special case handling
 *
 * Notes:
...
int getclass(int race) {
    ...
    ypos += 2;
    mvprintw(ypos++, 0, "     %-8s %4s   %15s %8s %4s", "class", "who", "", "magic", "cost");
 ...
 *   Key Tests: [Leader count assignment, power configuration, cost calculation, Human Warlord
 * special case] Dependencies: [Class configuration arrays, current nation structure, magic
 * system macros] Mock Requirements: [Class data arrays, nation structure, CHGMGK macro]
 ...
 */
int doclass(int tmp, int isupd /* true if update, false if interactive */
) {
    int cost;
    long x;
...
int nstartcst(void) /* to be used for new method */
{
    ...
    for (i = 0; i < CH_NUMBER; i++) {
        points += safe_int_to_float(Mcost[i]) * safe_int_to_float(spent[i])
                  / safe_int_to_float(Munits[i]);
    }
 ...
 *   Key Tests: [Race-specific calculations, location costs, late-start bonus, ORC special
 * cases] Dependencies: [Current nation structure, cost constants, game turn state] Mock
 * Requirements: [Nation with various attribute combinations, cost constants] Complexity: Simple
 * - Direct attribute-to-cost conversion with race branching
 *
 * Notes:
 *   - Thread safety: Read-only access to global state, generally safe
 ...
 * @last_documented: 2025-09-20
 */
int startcost(void) /* cant be used for npc nations yet!!! see below */
{
    float points; /* points */
