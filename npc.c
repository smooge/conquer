...
#define SEE_SECTOR(x, y, country)                                                              \
    ((magic(sct[x][y].owner, THE_VOID) != TRUE) || (magic(country, NINJA) == TRUE))
...
#define COUNT_ARMIES(army_nation, country)                                                     \
    (((magic(army_nation, THE_VOID) != TRUE) && (magic(army_nation, HIDDEN) != TRUE))          \
...
#define UNS_CITY_VALUE                                                                         \
    10 /* If there is an under-defended City                                                   \
        * the value is 500                                                                     \
        */
...
void prtattr(void) {
    ...
    for (y = (int)curntn->capy - 3; y < (int)curntn->capy + 4; y++) {
        ...
        for (x = (int)curntn->capx - 3; x < (int)curntn->capx + 4; x++)
            if (ONMAP(x, y)) {
                if (sct[x][y].altitude != WATER) {
                    printf("%5d %c%c%c ", attr[x][y], sct[x][y].vegetation, sct[x][y].altitude,
                           sct[x][y].designation);
                } else
                    printf("0      ~  ");
            }
    ...
    for (armynum = 0; armynum < MAXARM; armynum++)
        if (P_ASOLD > 0) {
            ...
            while (TRUE) {
                ...
                if ((x == P_AXLOC && y == P_AYLOC) || (!ONMAP(x, y) || (!is_habitable(x, y)))
...
void n_redes(int x, int y, int goldthresh, int metalthresh, int citythresh, double hunger) {
    ...
    if ((sptr->designation == DFARM) && (hunger > P_EATRATE * 1.5)
        ...
        if ((rand() % 2 == 0) && (curntn->mine_ability < 30))
            sptr->designation = DBLKSMITH;
        else if ((rand() % 2 == 0) && (sptr->people < 100) && (curntn->spoilrate > 15))
...
void redomil(void) {
    ...
    for (armynum = 1; armynum < MAXARM; armynum++)
        if (P_ASOLD > 0) {
            ...
            for (x = safe_int_to_short((int)P_AXLOC - 3);
                 x <= safe_int_to_short((int)P_AXLOC + 3); x++)
                for (y = safe_int_to_short((int)P_AYLOC - 3);
                     y <= safe_int_to_short((int)P_AYLOC + 3); y++)
            ...
            if (P_ATYPE == A_MILITIA) {
                ...
                if (((sct[P_AXLOC][P_AYLOC].designation != DTOWN)
                    ...
                    printf("\teliminating %s army %d as %d %d is des:%c alt:%c own:%d\n",
                           ...
                           sct[P_AXLOC][P_AYLOC].designation, sct[P_AXLOC][P_AYLOC].altitude,
                           sct[P_AXLOC][P_AYLOC].owner);
#endif /* DEBUG */
    ...
    check()
        ...
        for (armynum = 0; armynum < MAXARM;
             armynum++) if (P_ATYPE == getleader(curntn->class) - 1) {
        P_ASTAT = RULE;
        P_AXLOC = curntn->capx;
        P_AYLOC = curntn->capy;
        break;
    ...
    for (armynum = 1; armynum < MAXARM; armynum++)
        if ((P_ASOLD > 0) && (P_ATYPE != A_MILITIA) && (P_ATYPE < MINLEADER)
            ...
            if (magic(country, WARRIOR) == TRUE) /* WARRIOR power */
                ...
                curntn->tgold -= (TAKESECTOR + 20 - P_ASOLD) * *(u_encost + (P_ATYPE % UTYPE));
    ...
    if (curntn->tmil < ((4 * ideal) / 5)) {
        for (armynum = 1; armynum < MAXARM; armynum++)
            if ((done == FALSE) && (P_ASOLD == 0)) {
                ...
                P_ATYPE = safe_long_to_uchar(defaultunit(country));
                P_ASOLD = min((int)(ideal - curntn->tmil),
                              (int)(curntn->metals / (*(u_enmetal + (P_ATYPE % UTYPE)))));
        ...
        for (armynum = 1; done == FALSE && armynum < MAXARM; armynum++) {
            ...
            if ((sct[P_AXLOC][P_AYLOC].owner == country)
                && ((sct[P_AXLOC][P_AYLOC].jewels > 4) || (sct[P_AXLOC][P_AYLOC].metal > 4)
            ...
            /*minimum npc army is TAKESECTOR, merge them*/
                ...
                for (i = 1; free == FALSE && i < MAXARM; i++) {
                    if ((curntn->arm[i].sold > 0) && (curntn->arm[i].stat != ONBOARD)
                        ...
                        && (curntn->arm[i].stat != TRADED) && (curntn->arm[i].xloc == P_AXLOC)
                        && (curntn->arm[i].yloc == P_AYLOC)
                        && (curntn->arm[i].unittyp == P_ATYPE)) {
                        free = TRUE;
                        curntn->arm[i].sold += P_ASOLD;
#ifdef DEBUG
                        printf(
                            "\tMerge %ld men from army %d to make %ld troops in %s army %d \n",
    ...
    check()
        ...
        if (curntn->tgold
            > 0) for (x = safe_int_to_short(stx); x < endx;
                      x++) for (y = safe_int_to_short(sty); y < endy;
                                y++) if ((sct[x][y].owner == country)
                                         && ((sct[x][y].designation == DTOWN)
                                             || (sct[x][y].designation == DCITY)
                                             || (sct[x][y].designation == DCAPITOL))) {
        free = FALSE;
        for (armynum = 0; armynum < MAXARM; armynum++) {
            if ((P_ASOLD > 0) && (P_AXLOC == x) && (P_AYLOC == y) && (P_ATYPE == A_MILITIA)) {
                free = TRUE;
                break;
            }
        }
        ...
        if (free == TRUE) {
            ...
            if (ideal > 0) {
                if (magic(country, WARRIOR) == TRUE) { /* WARRIOR power */
                    curntn->tgold -= ((ideal - P_ASOLD) * *(u_encost + P_ATYPE)) / 2;
                } else {
...
void getdstatus(void) {
    ...
    if ((curntn->active == GOOD_6FREE) || (curntn->active == ISOLATIONIST)
        ...
        svhostile = 5;
    else if ((curntn->active == GOOD_4FREE) || (curntn->active == NEUTRAL_4FREE)
             || (curntn->active == EVIL_4FREE))
        svhostile = 10;
    else if ((curntn->active == GOOD_2FREE) || (curntn->active == NEUTRAL_2FREE)
             || (curntn->active == EVIL_2FREE))
        svhostile = 20;
    else if ((curntn->active == GOOD_0FREE) || (curntn->active == NEUTRAL_0FREE)
    ...
    for (x = 1; x < NTOTAL; x++)
        if (isntn(ntn[x].active)) {
            ...
            if ((ntn[x].tmil > 4 * curntn->tmil) && (ntn[x].score > 4 * curntn->score)) {
            ...
            for (armynum = 1; armynum < MAXARM; armynum++)
                if (ntn[nation].arm[armynum].sold > 0) {
                    ...
                    for (i = 1; (i < armynum) && !repeat; ++i)
                        if ((ntn[nation].arm[i].xloc == x) && (ntn[nation].arm[i].yloc == y))
                            repeat = TRUE;
                    if (!repeat)
                        total_sectors++;
                }
...
void nationrun(void) {
                ...
                if (isntn(ntn[x].active) && (curntn->dstatus[x] > HOSTILE)) {
                    if (100 * (curntn->tmil * (curntn->aplus + 100))
                            / ((curntn->tmil * (curntn->aplus + 100))
                               + (ntn[x].tmil * (ntn[x].dplus + 100)))
                        > rand() % 100) {
                        /*attacker*/
                        for (armynum = 1; armynum < MAXARM; armynum++)
                            if ((P_ASOLD > 0) && (P_ATYPE != A_MILITIA) && (P_ASTAT != ONBOARD)
                                && (P_ASTAT != TRADED) && (P_ASTAT < NUMSTATUS)
                                && (P_ASTAT != GENERAL))
                                P_ASTAT = ATTACK;
                        atkattr();
                    } else { /*defender*/
    ...
    for (loop = 1; loop < 5; loop++) {
        ...
        printf("\tnpcredes(): country %s gold=%d metal=%d, city=%d hunger=%f\n", curntn->name,
               goldthresh, metalthresh, citythresh, hunger);
#endif /* DEBUG */
        ...
        for (x = stx; x < endx; x++)
            for (y = sty; y < endy; y++)
                if ((sct[x][y].owner == country) && (is_habitable(x, y))
                    && ((isnotpc(curntn->active)) || (sct[x][y].designation == DNODESIG))) {
        ...
        if (spread.civilians + 2 * curntn->tmil > 0) {
            hunger = safe_long_to_float(spread.food)
                     / ((safe_long_to_float(spread.civilians + 2 * curntn->tmil)));
        } else
            hunger = 0.0;
        if (hunger < P_EATRATE) {
            goldthresh++;
        ...
        for (x = stx; x < endx; x++)
            for (y = sty; y < endy; y++)
                if ((sct[x][y].owner == country)
                    && ((sct[x][y].designation == DTOWN) || (sct[x][y].designation == DCITY)
                        || (sct[x][y].designation == DCAPITOL)
                        || (sct[x][y].designation == DFORT))
                    && (sct[x][y].fortress < 10) && (curntn->tgold > 10000) && (rand() % 5 == 0)
    ...
    check() if (magic(country, VAMPIRE) != TRUE) {
        i = 0;
    ...
    for (x = stx; x < endx; x++)
        for (y = sty; y < endy; y++) {
            if ((sct[x][y].owner != country) && (sct[x][y].owner != 0)
                && (abs(x - (int)curntn->capx) > 2) && (abs(y - (int)curntn->capy) > 2)
            ...
            if (sct[x][y].owner == nation) {
                if (SEE_CITIES(nation, country)) {
                    if (((sct[x][y].designation == DCITY) || (sct[x][y].designation == DCAPITOL)
                        ...
                        solds = 0;
                        for (armynum = 1; armynum < MAXARM; armynum++)
                            if ((ntn[country].arm[armynum].sold > 0) && (abs(AXLOC - x) <= 1)
                                && (abs(AYLOC - y) <= 1))
                                solds += ASOLD;
 ...
 *   - Expansion doctrine: Implements efficient territorial acquisition through weakness
 * targeting
 * @last_documented: 2025-09-19
 */
...
void n_survive(void) {
    ...
    for (nation = 1; nation < NTOTAL; nation++)
        if ((isntn(ntn[nation].active))
            && ((ntn[nation].dstatus[country] >= WAR) || (curntn->dstatus[nation] >= WAR))) {
            if (COUNT_ARMIES(nation, country)) {
                for (armynum = 1; armynum < MAXARM; armynum++)
                    if ((ntn[nation].arm[armynum].sold > 0)
                        ...
                        if ((ntn[nation].arm[armynum].xloc == capx)
                            ...
                            attr[capx][capy] +=
                                safe_long_to_int(2 * ntn[nation].arm[armynum].sold);
                        } else {
                            attr[ntn[nation].arm[armynum].xloc]
                                [ntn[nation].arm[armynum].yloc] +=
                                safe_long_to_int(ntn[nation].arm[armynum].sold);
                        }
                    }
            } else {
 ...
 *   Key Tests: War vs jihad intensity differences, multi-enemy coordination, constraint
 * application Dependencies: Full diplomatic system, nation management, all attractiveness
 * subsystems Mock Requirements: Multiple nations, varied diplomatic relations (WAR/JIHAD),
 * world geography Complexity: Moderate - Orchestrates multiple offensive subsystems with
 * escalation scaling
 *
 * Notes:
 ...
 *
 * Testing Notes:
 *   Category: A (Unit) - Simpler coordination with fewer dependencies than military
 * coordinators Approach: Unit testing with peaceful diplomatic scenarios and expansion
 * opportunities Key Tests: Triple expansion priority, constraint application, no military
 * interference Dependencies: Minimal - territorial data, diplomatic constraints, emergency
 * protocols Mock Requirements: World geography, territorial ownership, basic diplomatic state
