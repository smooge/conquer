...
void mymove(void) {
    ...
    while (done == FALSE) {
        ...
        switch (getch()) {
            ...
            case ' ':
                valid = FALSE;
                if (mveleft > 0 && armornvy == ARMY && sct[XREAL][YREAL].altitude == WATER
                    && P_ASTAT == FLIGHT)
        ...
        if (valid == TRUE) {
            if (armornvy == ARMY) {
                ...
                } else if ((movecost[XREAL][YREAL] < 0) || (movecost[XREAL][YREAL] > mveleft)) {
                ...
                if (abs(movecost[XREAL][YREAL]) > mveleft) {
                    ...
                    xcurs = safe_int_to_short(oldxcurs);
                    ycurs = safe_int_to_short(oldycurs);
                } else if (movecost[XREAL][YREAL] >= 0 || movecost[XREAL][YREAL] == -2) {
                    /* LAND OF SOME TYPE */
                    valid = FALSE;
        ...
        if ((valid == TRUE) && (done == FALSE)) {
                    ...
                    if (Tnation != country)
                        for (Tarmynum = 0; Tarmynum < MAXARM; Tarmynum++)
                            if ((ntn[Tnation].arm[Tarmynum].sold > 0)
                                ...
                                total =
                                    safe_long_to_int(total + ntn[Tnation].arm[Tarmynum].sold);
            } else {
            ...
            if ((armornvy == ARMY) && (P_ASTAT != SCOUT) && (P_ATYPE != A_NINJA)
                ...
                if (groupmen + othermen < total) {
                    ...
                    P_AMOVE = safe_long_to_uchar(P_AMOVE
                                                 - total * curntn->maxmove
                                                       * *(unitmove + (P_ATYPE % UTYPE))
        ...
        if (done == FALSE) {
            ...
            if (armornvy == ARMY) {
                ...
                mvprintw(LINES - 2, 0, "MOVESCREEN: move left: %d  move cost is %d", P_NMOVE,
                         abs(movecost[XREAL][YREAL]));
            clrtoeol();
            standend();
            mvaddstr(LINES - 1, 0, "HIT SPACE IF DONE");
            clrtoeol();
    ...
    if (armornvy == ARMY) {
        ...
        if (((P_ATYPE < MINLEADER) || (P_ASTAT == GENERAL)) && (P_ASOLD > 0)) {
            ...
            if ((groupmen >= TAKESECTOR) && (SOWN == 0)) {
                ...
                refresh();
                sleep(2);
            } else if ((SOWN != country) && (groupmen >= TAKESECTOR)
                ...
                clear_bottom(0);
                mvprintw(LINES - 3, 0,
                         "army has too few men (%ld) to take sector (need %ld) - hit any key",
                         groupmen, TAKESECTOR);
                refresh();
                getch();
            }
