 ...
 *   Complexity: Complex - Multi-system integration with financial transactions and cascading
 * effects
 * @last_documented: 2025-09-20
 */
void diploscrn(void) {
    ...
    while (1) {
        ...
        for (i = count2; i < NTOTAL && count < MAXONSCR; i++)
            if ((isntnorp(ntn[i].active)) && (i != country)) {
                ...
                standend();
                if ((ntn[i].dstatus[country] == WAR) || (ntn[i].dstatus[country] == JIHAD))
                    standout();
        ...
        mvprintw(LINES - 6, COLS / 2 - 31,
                 "HIT 'B' KEY TO BRIBE NPC NATION (cost=$%ld per 1000 Mil.)", BRIBE);
        ...
        if ((k != 'B') && (k != '\r') && (k != '\n')) {
            ...
            for (; i < NTOTAL && (!isntnorp(ntn[i].active) || i == country); i++)
                ;
            if (i == NTOTAL)
                count2 = safe_int_to_short(1);
            else
                count2 = safe_int_to_short(i);
            continue;
        }
        if (k == 'B') {
            ...
            if (isgod == TRUE) {
                ...
                bribecost = BRIBE;
            mvprintw(LINES - 3, 0, "This will cost %ld gold talons. continue (y or n)",
                     bribecost);
            refresh();
        ...
        if ((isgod == FALSE) && (curntn->tgold < BREAKJIHAD)
            && (((curntn->dstatus[nation] == TREATY) && (ntn[nation].dstatus[country] < WAR))
                || (curntn->dstatus[nation] == JIHAD))) {
            mvprintw(LINES - 1, 0, "Sorry, need %ld talons to change status with ntn %s",
                     BREAKJIHAD, ntn[nation].name);
            clrtoeol();
            ...
            if ((temp <= UNMET) || (temp > JIHAD) || ((isgod == FALSE) && (temp == UNMET))) {
                    ...
                    if (ntn[i].dstatus[nation] == TREATY) {
                        ...
                        if (mailopen(country) == (-1)) {
                            ...
                            switch (rand() % 4) {
                                case 0:
                                    fprintf(fm,
                                            "You just attacked my friend %s - your loss!!!\n",
                                            ntn[nation].name);
                                    break;
                                case 1:
                                ...
                                case 3:
                                    fprintf(fm,
                                            "We deplore your aggression against %s and are "
                                            "taking\n",
                                            ntn[nation].name);
...
void change(void) {
    ...
    while (1) {
        ...
        if (isgod == TRUE)
            ...
            mvaddstr(LINES - 3, COLS / 2 - 14, "HIT ANY OTHER KEY TO CONTINUE");
        standend();
        refresh();
        switch (getch()) {
            case '1': /*get name*/
            ...
            case '4': /* charity */
                ...
                if (intval < 0) {
                    ...
                    if (2 * (intval - (int)curntn->charity) + (int)curntn->popularity > 100) {
                        ...
                        curntn->popularity +=
                            safe_int_to_uchar(2 * (intval - (int)curntn->charity));
            ...
            case '6': /* combat bonus */
                ...
                if (curntn->race == ORC)
                    cost *= 3;
                mvprintw(LINES - 1, 0,
                         "Do You Wish Spend %ld Metal On Defense (enter y or n):", cost);
                refresh();
                if (getch() == 'y') {
                    if (curntn->metals > cost) {
                        curntn->dplus += 1;
            ...
            case '8':
                if (isgod == TRUE) {
                    ...
                    if (getch() == 'y') {
                        ...
                        destroy(country);
                        fclose(fnews);
                        /* Sort news file using native C implementation (replaces
                         * system("conqsort filename filename")) */
    ...
    while (done == FALSE) {
        ...
        if (strncmp(line, "DONE", 4) == 0)
            ...
            while (strncmp(line, "END", 3) != 0) {
                ...
                lineno++;
                if (lineno > LINES - 3)
                    strncpy(line, "END", sizeof(line) - 1);
                else
                    fgets(line, 80, fp);
            }
            standout();
...
readoldp: /* label to jump if reading old pages */
    ...
    for (i = TURN - 1; i >= 0 && i >= TURN - MAXNEWS; i--) {
        snprintf(line, sizeof(line), "   %d) %s of Year %d", TURN - i, PSEASON(i), YEAR(i));
        /* align all strings */
