...
void update(void) {
...
#ifdef MONSTER
        check() monster(); /* update monster nations */
    check()
#endif

        check() combat(); /* run combat */
    check() updcapture(); /* capture unoccupied sectors */
    ...
    for (country = 1; country < NTOTAL; country++)
        if (isntn(ntn[country].active)) {
            if (ntn[country].tciv < 100 && ntn[country].tmil < TAKESECTOR)
                destroy(country);
        }
    ...
    /* Sort news file using native C implementation (replaces system("conqsort filename
     * filename")) */
...
int attract(int x, int y, int race) {
    ...
    if (sptr->tradegood != TG_none && *(tg_stype + sptr->tradegood) == designation) {
...
int armymove(int armynum) {
    ...
    if (leadflag) { /* Move based on unattached soldiers */
        for (i = 0; i < MAXARM; i++)
            if ((curntn->arm[i].unittyp < MINLEADER) && (curntn->arm[i].stat != MILITIA)
                && (curntn->arm[i].stat != ONBOARD) && (curntn->arm[i].stat != GARRISON)
        ...
        for (x = safe_long_to_int(P_AXLOC - 4 + menok * 2);
             x <= safe_long_to_int(P_AXLOC + 4 - menok * 2); x++)
            for (y = safe_long_to_int(P_AYLOC - 4 + menok * 2);
                 y <= safe_long_to_int(P_AYLOC + 4 - menok * 2); y++) {
                if (ONMAP(x, y)) {
    ...
    if (sum == 0) { /* nowhere to go */
        ...
        where = rand() % sum;
        for (x = 0; x < MAXARM; x++)
            if ((curntn->arm[x].unittyp < MINLEADER) && (curntn->arm[x].stat != MILITIA)
                && (curntn->arm[x].stat != GARRISON) && (curntn->arm[x].stat != ONBOARD)
                && (curntn->arm[x].stat != TRADED) && (curntn->arm[x].stat < NUMSTATUS)) {
        ...
        if (x != MAXARM)
            for (x = 0; x < MAXARM; x++) {
                if ((curntn->arm[x].unittyp < MINLEADER) && (curntn->arm[x].stat < NUMSTATUS)
                    && (curntn->arm[x].sold >= 0) && (curntn->arm[x].stat != MILITIA)
                    && (curntn->arm[x].stat != GARRISON) && (curntn->arm[x].stat != SIEGED)
                    && (curntn->arm[x].stat != SCOUT) && (curntn->arm[x].stat != ONBOARD)
                    && (curntn->arm[x].stat != TRADED) && (curntn->arm[x].unittyp != A_ZOMBIE)
                    && (P_AXLOC == curntn->arm[x].xloc) && (P_AYLOC == curntn->arm[x].yloc)) {
...
void cheat(void) {
    ...
    npcavg = bonus / count;
    for (x = 1; x < NTOTAL; x++)
        if ((realnpc[x] == TRUE) && (ntn[x].score < avgscore) && (ntn[x].race != ORC)
...
void updexecs(void) {
    ...
    check() attr = (int **)m2alloc(MAPX, MAPY, sizeof(int));
    check() newpop = (long **)m2alloc(5, MAPY, sizeof(long));
    check()

        for (country = 0; country < NTOTAL; country++) if (isntn(ntn[country].active))
                    ...
                    for (i = (int)ntn[country].arm[armynum - 1].xloc - 1;
                        ...
                        for (j = (int)ntn[country].arm[armynum - 1].yloc - 1;
                            ...
                            if (ONMAP(i, j) && (sct[i][j].altitude != WATER)
                                && (sct[i][j].altitude != PEAK) && (sct[i][j].owner != country)
                                && (rand() % 3 == 0)) {
    ...
    for (armynum = 0; armynum < MAXARM; armynum++) {
        if ((P_ASOLD > 0) && (sct[P_AXLOC][P_AYLOC].altitude == WATER))
            printf("ERROR line %d... %s army %d in water (army %d: x: %d y: %d)\n", __LINE__,
                   ntn[country].name, armynum, armynum - 1, ntn[country].arm[armynum - 1].xloc,
                ...
                if (P_ATYPE < MINLEADER) {
                    if ((ispc(curntn->active) && (P_ASOLD >= TAKESECTOR))
                        ...
                        if (sptr->owner == 0) {
                            ...
                            if ((isntn(curntn->active)) && (isntn(ntn[sptr->owner].active))) {
#ifdef HIDELOC
...
void updsectors(void) {
            ...
            if (sptr->people >= ABSMAXPEOPLE) {
                ...
                if (sptr->people * sptr->jewels > 2 * (rand() % 100) * TOMUCHMINED)
                    if (sptr->designation == DGOLDMINE)
                        sptr->jewels--;
            } else if ((sptr->people > TOMANYPEOPLE) && (sptr->designation != DTOWN)
                       && (sptr->designation != DCAPITOL) && (sptr->designation != DCITY)) {
    ...
    for (country = 1; country < NTOTAL; country++) {
        ...
        if (isntn(curntn->active)) {
            ...
            curntn->popularity =
                safe_long_to_uchar(min(curntn->popularity + 5 * charity, MAXTGVAL));

            /* charity adjustment to poverty; rounding upward */
            if (curntn->poverty < (charity + 1) / 2)
                curntn->poverty = 0;
            else
                curntn->poverty -= safe_long_to_uchar((charity + 1) / 2);
            ...
            if (spread.civilians > 0)
                curntn->inflation +=
                    safe_long_to_short((curntn->tmil * 100 / spread.civilians - 15) / 5);
            ...
            for (armynum = 0; armynum < MAXARM; armynum++)
                if (P_ASOLD > 0) {
                    ...
                    if (disarray)
                        ...
                        switch (A->stat) {
                            case MARCH:
                                A->smove = safe_int_to_uchar(
                                    (curntn->maxmove * *(unitmove + (AT % UTYPE))) / 5);
                                break;
                            case MILITIA:
                            case ONBOARD:
                                A->smove = 0;
                                        ...
                                        for (nation = 0; nation < NTOTAL; nation++) {
                                            for (army2 = 0; army2 < MAXARM; army2++)
                                                if ((ntn[nation].arm[army2].xloc == AX)
                                                    ...
                                                    if (ntn[nation].arm[army2].unittyp
                                                        == A_SIEGE)
                                                        asmen += safe_long_to_int(
                                                            3 * ntn[nation].arm[army2].sold);
                                                    else
                                                        asmen += safe_long_to_int(
                                                            ntn[nation].arm[army2].sold);
                                                }
                                        }
                                        ...
                                        for (army2 = 0; army2 < MAXARM; army2++)
                                            if ((ntn[nation].arm[army2].xloc == AX)
                                                ...
                                                if (ntn[nation].arm[army2].unittyp == A_MILITIA)
                                                    dsmen += safe_long_to_int(
                                                        ntn[nation].arm[army2].sold / 2);
                                                else
                                                    dsmen += safe_long_to_int(
                                                        ntn[nation].arm[army2].sold);
                                            }
                            ...
                            default:
                                A->smove = safe_int_to_uchar(
                                    (curntn->maxmove * *(unitmove + (AT % UTYPE))) / 10);
                                break;
                        }

                    /* empower flight */
                    if ((avian(AT) == TRUE) && (A->stat != ONBOARD) && (A->stat < NUMSTATUS))
                        A->stat = FLIGHT;

                    if ((magic(country, ROADS) == TRUE) && (sct[AX][AY].owner != country)) {
                        if (A->smove > 7)
                            A->smove -= 4;
                        else if (A->smove > 4)
                            A->smove = 4;
            ...
            for (nvynum = 0; nvynum < MAXNAVY; nvynum++) {
                ...
                if (P_NWSHP != 0 || P_NMSHP != 0 || P_NGSHP != 0) {
                    ...
                    if (disarray)
                        ...
                        P_NMOVE = safe_int_to_uchar((fltspeed(nvynum) * P_NCREW) / SHIPCREW);
                ...
                for (x = 0; x < MAPX; x++)
                    for (y = 0; y < MAPY; y++) {
                        ...
                        if ((sptr->owner == country)
                            && ((sptr->designation == DTOWN) || (sptr->designation == DCAPITOL)
                                || (sptr->designation == DCITY))
                            && (curntn->tfood < 0)) {
 ...
 *
 * Mathematical Model:
 * EQUILIBRIUM(sector) = Attractiveness(sector) / Total_Area_Attractiveness *
 * Total_Area_Population MOVEMENT_DELTA = (EQUILIBRIUM - Current_Population) / 5 NEW_POPULATION
 * = Current_Population + MOVEMENT_DELTA
 *
