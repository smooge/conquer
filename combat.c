...
void fight(void) {
    ...
    for (i = 0; i < count; i++)
        if (owner[i] > (-1)) {
            ...
            if (((ntn[owner[i]].arm[unit[i]].unittyp == A_MERCENARY)
                ...
                && (ntn[owner[i]].arm[unit[i]].stat < NUMSTATUS) && (rand() % 100 < 15)) {
    ...
    for (i = 0; i < count; i++)
        if (owner[i] > (-1)) {
            if ((((odds > 200) && (side[i] == DFND)) || ((odds < 100) && (side[i] == ATKR)))
                && ((ntn[owner[i]].arm[unit[i]].unittyp == A_MERCENARY)
                    || (ntn[owner[i]].arm[unit[i]].unittyp == A_ORC)
                    || (ntn[owner[i]].arm[unit[i]].unittyp == A_GOBLIN))
                && (ntn[owner[i]].arm[unit[i]].stat < NUMSTATUS) && (rand() % 100 < 30)) {
                if (ispc(ntn[owner[i]].active)) {
                    if (mailopen(owner[i]) != (-1)) {
                        fprintf(fm, "Message to %s from Conquer\n\n", ntn[owner[i]].name);
        ...
        if (owner[i] > (-1)) {
            if (ISCITY(sct[xspot][yspot].designation)) {
                if ((ntn[owner[i]].arm[unit[i]].unittyp == A_CAVALRY)
                    ...
                    if (side[i] == ATKR)
                        j += safe_long_to_int(troops[i]);
                    else if (side[i] == DFND)
                        k += safe_long_to_int(troops[i]);
                }
            }
        }
    ...
    for (i = 0; i < count; i++)
        if (owner[i] > (-1)) {
            if (j > 0)
                abonus += safe_long_to_int((15 * j * troops[i]) / asold);
            if (k > 0 && dsold > 0)
                dbonus += safe_long_to_int((15 * k * troops[i]) / dsold);
        }

    abonus = safe_long_to_int(abonus / asold);
    if (dsold > 0)
        dbonus = safe_long_to_int(dbonus / dsold);
    ...
    for (i = 0; i < count; i++)
        if (owner[i] > (-1)) {
            if (fort_val(&sct[xspot][yspot]) != 0) {
                ...
                if ((ntn[owner[i]].arm[unit[i]].unittyp == A_CATAPULT) && (side[i] == DFND))
                ...
                else if ((ntn[owner[i]].arm[unit[i]].unittyp == A_SIEGE) && (side[i] == ATKR)) {
    ...
    if (astr > dstr * 100)
        ...
        odds = safe_long_to_int((astr * 100) / dstr);
            ...
            if (side[i] == ATKR) {
                if (ntn[owner[i]].arm[unit[i]].unittyp >= MINLEADER) {
                    ...
                    if ((ntn[owner[i]].arm[unit[i]].unittyp == A_MILITIA)
                        && (retreatside == ATKR)) {
                        sct[ntn[owner[i]].arm[unit[i]].xloc][ntn[owner[i]].arm[unit[i]].yloc]
...
int cbonus(int num) {
    ...
    if (((magic(country, DESTROYER) == 1) || (magic(country, DERVISH) == 1))
        && ((sct[xspot][yspot].vegetation == ICE) || (sct[xspot][yspot].vegetation == DESERT)))
        armbonus += 30;
...
void fdxyretreat(void) { /* finds retreat location */
    ...
    if ((sct[xsctr][ysctr].designation == DTOWN) || (sct[xsctr][ysctr].designation == DCAPITOL)
                        ...
                        if (rand() % 100 < cptpct / 2) {
                            ...
                            if (which == ATKR) {
                                if (dhold) {
                                    awcapt++;
                                    ahold -= (shipsize + 1);
                                    capture(QWAR, DFND, shipsize,
                                            safe_long_to_int(rand() % dhold + 1));
                                    NSUB_WAR(1)
                                }
                            } else {
                                if (ahold) {
                                    dwcapt++;
                                    dhold -= (shipsize + 1);
                                    capture(QWAR, ATKR, shipsize,
                                            safe_long_to_int(rand() % ahold + 1));
                                    NSUB_WAR(1)
                                }
                            }
                            if (which == ATKR)
                ...
                for (i = 0; i < P_NGAL(shipsize); i++)
                    if (rand() % 100 < Ploss) {
                        if (rand() % 100 < cptpct) {
                            ...
                            if (P_NARMY != MAXARM) {
                                armynum = P_NARMY;
                                k = safe_long_to_int(P_ASOLD * (shipsize + 1)
                                                     / (ghold + shipsize + 1));
                                if (P_ATYPE < MINLEADER) {
                                    ...
                                    if (which == ATKR)
                                        akcrew += safe_long_to_int(P_ASOLD);
                                    else
                                        dkcrew += safe_long_to_int(P_ASOLD);
                                    P_ASOLD = 0;
                                    P_NARMY = MAXARM;
                                }
                            }
                            ...
                            if (P_NARMY != MAXARM) {
                                armynum = P_NARMY;
                                k = safe_long_to_int(P_ASOLD * (shipsize + 1) * Ploss
                                                     / (ghold * 100));
                                if (P_ATYPE < MINLEADER) {
                                    P_ASOLD -= k;
                                    if (which == ATKR)
...
void capture(int type, int to, int shipsize, int holdcount) {
    ...
    switch (type) {
        case QWAR:
            (void)NADD_WAR(1) break;
        case QGAL:
            (void)NADD_GAL(1) break;
        case QMER:
            (void)NADD_MER(1) break;
        default:
