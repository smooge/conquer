...
void get_display_mode(short *dmode, short *hmode, short *odmode, short *ohmode) {
    ...
    clrtoeol();
    mvaddstr(LINES - 2, 0,
             "highlight option: (o)wned, (a)rmy, (y)our Army, move (l)eft, (s)pecial,(x)=none");
    clrtoeol();
    if (odmode != NULL) {
...
char get_display_for(int x, int y, short dmode) {
    ...
    if ((magic(sct[x + xoffset][y + yoffset].owner, THE_VOID) == TRUE)
        ...
        && (country != sct[x + xoffset][y + yoffset].owner) && (magic(country, NINJA) != TRUE)
        && (country != 0)) {
        ch = '?';
    } else {
        switch (dmode) {
            case DI_FOOD: /*food */
                if (tofood(&sct[x + xoffset][y + yoffset], country) == 0)
                    ...
                    ch =
                        safe_int_to_char(tofood(&sct[x + xoffset][y + yoffset], country) + '0');
                else
                    ch = '+';
                break;
            ...
            case DI_DESI: /*designation*/
                if (sct[x + xoffset][y + yoffset].owner == 0) {
                    if (tofood(&sct[x + xoffset][y + yoffset], country) != 0)
                        ...
                        ch = sct[x + xoffset][y + yoffset].vegetation;
                } else if ((country == 0) || (sct[x + xoffset][y + yoffset].owner == country))
            ...
            case DI_PEOP: /*People*/
                if (sct[x + xoffset][y + yoffset].altitude == WATER)
                    ...
                    ch = safe_long_to_char((50 + sct[x + xoffset][y + yoffset].people) / 100
                                           + '0');
                break;
            case DI_GOLD: /*Gold*/
                if (sct[x + xoffset][y + yoffset].altitude == WATER)
...
void highlight(int x, int y, short hmode) {
    ...
    switch (hmode) {
        ...
        case HI_YARM: /* your armies */
            for (armynum = 0; armynum < MAXARM; armynum++)
                if ((P_ASOLD != 0) && (P_AXLOC == x + xoffset) && (P_AYLOC == y + yoffset))
                    break;
            if (armynum < MAXARM)
                standout();
            break;
        case HI_GOOD: /* trade goods */
            if (tg_ok(country, &sct[x + xoffset][y + yoffset])
                ...
                && (magic(sct[x + xoffset][y + yoffset].owner, THE_VOID) != TRUE || country == 0
 ...
 */
void coffmap(void) {
    if (xcurs < 0 || (XREAL != 0 && xcurs == 0) || (ycurs < 0) || (YREAL != 0 && ycurs == 0)
        || xcurs >= SCREEN_X_SIZE - 1 || ycurs >= SCREEN_Y_SIZE - 1 || XREAL >= MAPX
        || YREAL >= MAPY) {
        centermap();
        redraw = PART;
    }
        ...
        if ((P_NMSHP != 0) || (P_NWSHP != 0) || (P_NGSHP != 0))
            for (i = (int)P_NXLOC - xoffset - NAVYSEE;
                ...
                for (j = (int)P_NYLOC - yoffset - NAVYSEE;
                    ...
                    if (ONMAP(i + xoffset, j + yoffset) && i >= 0 && j >= 0 && i < SCREEN_X_SIZE
                        && j < SCREEN_Y_SIZE)
                        HAS_SEEN(i, j) = TRUE;
