...
void makeworld(int rflag) { /* TRUE if you wish to read in a map from mapfiles */
    ...
    const char *pattern_ptrs[] = {patterns[0], patterns[1], patterns[2],
    ...
    while (TRUE) {
        ...
        if (strlen(newstring) == 0) {
            ...
            if (remake == FALSE) {
                ...
                snprintf(tempc, sizeof(tempc), "The demi-god %s will continue to reign.",
                         ntn[0].leader);
                newmsg(tempc);
                sleep(1);
            ...
            if (getpwnam(newstring) != NULL) {
                (void)snprintf(tempc, sizeof(tempc),
                               "The demi-god %.200s may administrate this new world.",
                               newstring);
                newmsg(tempc);
                ...
                (void)snprintf(tempc, sizeof(tempc),
                               "Their is no mortal named %.200s on this system.", newstring);
                newerror(tempc);
            }
        } else {
 ...
 *   Key Tests: Water percentage accuracy, mountain distribution, vegetation patterns,
 * geographical rules Dependencies: Global world dimensions, pwater setting, sector array,
 * random number generator Mock Requirements: Controlled random seed, memory allocation system,
 * display system Complexity: Complex - Most sophisticated algorithm in file, heavy
 * computational load
 *
 * Notes:
...
void createworld(void) { /* create world */
    ...
    mvprintw(11, 0, "Hills and Mountains....  %ld out of %ld sectors", nmountains,
             (long)NUMSECTS);
...
void populate(void) {
    ...
    mvprintw(13, 0, "Placing %d lizard cities, %d pirates, %d savages, and %d nomad tribes",
    ...
    while ((nlizards + npirates + nbarbarians + nnomads > 0) && (loopcnt++ < 5000)) {
        ...
        switch (curntn->active) {
            ...
            case NPC_PIRATE:
                ...
                P_NARMY = MAXARM;
                shipsize = N_LIGHT;
                (void)NADD_WAR(safe_int_to_short(rand() % 5 + 2)) shipsize = N_MEDIUM;
                (void)NADD_WAR(safe_int_to_short(rand() % 3 + 1)) shipsize = N_HEAVY;
                (void)NADD_WAR(safe_int_to_short(rand() % 2)) P_NCREW = SHIPCREW;
                nvynum++;
                break;
            case NPC_NOMAD:
                nnomads--;
    ...
    for (country = 1; country < NTOTAL; country++) {
        ...
        while (armynum < MAXARM) {
            ...
            if (is_habitable(x, y) && sct[x][y].owner == 0) {
                ...
                P_ASTAT = ATTACK;
                P_ATYPE =
                    safe_int_to_uchar(MINMONSTER + rand() % (MAXMONSTER - MINMONSTER + 1));
        ...
        if (line[0] != '#') {
            ...
            int result =
                sscanf(line, "%9s %9s %c %c %c %hd %hd %hd %ld %ld %d %hd %c %d %d %hd",
                       ...
                       &ntn[cnum].tgold, &ntn[cnum].tmil, &points, &short2, &allign, &xloc,
                       &yloc, &class);
            if (result != 16) {
            ...
            if (cnum > MAPX * MAPY / NPC * (100 - pwater) / 100) {
                snprintf(line, sizeof(line), "World too small to add npc nation %d %s", cnum,
                         curntn->name);
                newerror(line);
                continue;
            }
            if (isactive(ntn[cnum].active)) {
                snprintf(line, sizeof(line),
                         "Not enough available nations to add npc nation %d %s", cnum,
                         curntn->name);
                newerror(line);
                continue;
            }
