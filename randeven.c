 ...
 */
int disolve(int percent, int target, int ispsnt /* true/false */
) {
    ...
    if (ispsnt) {
        for (new = 0; new < NTOTAL; new++) {
            if ((ntn[new].race == ntn[target].race) && (ntn[new].active == NPC_PEASANT)) {
                printf("\tntn %s peasants of same type as target %d\n", ntn[new].name, target);
                snprintf(eventstr, sizeof(eventstr), "rebellion joins nation %s",
                         ntn[new].name);
                return (new);
            }
        }
        ...
        for (i = 0; i < MAPX; i++)
            for (j = 0; j < MAPY; j++) {
                if (sct[i][j].owner == target) {
                    if ((sct[i][j].designation == DCITY) || (sct[i][j].designation == DTOWN)
                        || ((sct[i][j].people >= 2000)
...
#else
    snprintf(eventstr, sizeof(eventstr), "new nation %s created at %d,%d", ntn[new].name, realx,
             realy);
#endif /* HIDELOC */
        ...
        if (split > 0)
            for (i = realx - dist; i < realx + dist; i++)
                for (j = realy - dist; j < realy + dist; j++) {
                    if (ONMAP(i, j) && (split > 0) && (sct[i][j].designation != DCAPITOL)
    ...
    if (!ispsnt)
        for (armynum = narmynum; armynum < MAXARM; armynum++) {
            ...
            if ((ntn[target].arm[armynum].sold > 0)
                && (sct[ntn[target].arm[armynum].xloc][ntn[target].arm[armynum].yloc].owner
                    == new)) {
...
void randomevent(void) {
    ...
    for (country = 0; country < NTOTAL; country++) {
        ...
        printf("TEMP: %s chance of revolt is %d (tax=%d prest=%d)\n", curntn->name, x,
               curntn->tax_rate, curntn->prestige);
#endif /* DEBUG */
            ...
            switch (event) {
                ...
                case 13:
                    ...
                    snprintf(eventstr, sizeof(eventstr),
                             "all flee, 30%% die in 1 sector range");
                ...
                case 27:
                    ...
                    strncpy(eventstr, "40% of populace & armies in towns die",
                            sizeof(eventstr) - 1);
                    eventstr[sizeof(eventstr) - 1] = '\0';
                    for (i = 0; i < MAPX; i++)
                        for (j = 0; j < MAPY; j++)
                    ...
                    if ((newpower = getmagic(M_CIV)) != 0L) {
                        for (i = S_CIV; i < S_CIV + E_CIV; i++)
                            if (powers[i] == newpower) {
                                snprintf(eventstr, sizeof(eventstr),
                                         "nation %s gets civilian power %s", curntn->name,
                                         pwrname[i]);
                            }
                        exenewmgk(newpower);
                    } else
                ...
                case 31: /*new magic item + RANDOM POWER*/
                    ...
                    if ((newpower = getmagic(M_MIL)) != 0) {
                        for (i = S_MIL; i < S_MIL + E_MIL; i++)
                            if (powers[i] == newpower)
                                snprintf(eventstr, sizeof(eventstr),
                                         "nation %s gets military power %s", curntn->name,
                                         pwrname[i]);
                        exenewmgk(newpower);
                    } else
                        done = FALSE;
                ...
                case 36:
                    ...
                    if (done == TRUE) {
                        if (magic(country, newpower) == TRUE) {
                            ...
                            removemgk(newpower);
                            snprintf(eventstr, sizeof(eventstr), "nation %s loses %s power",
 ...
 * @last_documented: 2025-09-20
 */
void peasant_revolt(int *newnation) /* peasant revolt */
{
    register int i, j;
    int armynum;
 ...
 */
int other_revolt(/* return reason and new nation number*/
                 int *new) {
 ...
 *
 * Testing Notes:
 *   Category: B (Integration) - Requires map state, population system, and notification
 * integration Approach: Integration testing with mock map sectors and population distributions
...
struct s_sector *rand_sector(void) {
    ...
    abrt() return (NULL); /* stop lint from complaining */
}
