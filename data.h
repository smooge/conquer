...
extern int	move_file(void), land_2reachp(void), land_reachp(void), canbeseen(int x, int y);
extern int	water_reachp(void), markok(void), is_habitable(void), parse(int ch);
extern int	units_in_sector(int x, int y, int nation), num_powers(void), tofood(struct s_sector *sptr, int cntry), mailopen(void);
...
extern int	unitvalid(void),orctake(void),fort_val(struct s_sector *sptr);
...
extern void	destroy(void), updmove(int race, int country), spreadsheet(void), mailclose(void);
