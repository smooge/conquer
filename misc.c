...
int land_2reachp(int ax, int ay, int move_points) {
...

    {
        ...
        for (i = 0; i < 8; i++) {
            ...
            if ((own = sct[x][y].owner) > 0 && ntn[own].dstatus[moving_country] >= WAR
                && x != bx && y != by && solds_in_sector(x, y, own) > 0) {
 ...
 *
 * Testing Notes:
 *   Category: B - Integration testing with land_2reachp() - Depends on map state - Medium
 * complexity Approach: Test valid paths, blocked paths, terrain constraints, movement point
 * limits Dependencies: Requires initialized map (sct[][]), land_2reachp(), m2alloc()
...
int is_habitable(int x, int y) {
    ...
    if (((temp = sct[x][y].vegetation) == BARREN) || (temp == LT_VEG) || (temp == GOOD)
        || (temp == WOOD) || (temp == FOREST))
        return (TRUE);
...
int units_in_sector(int x, int y, int nation) {
    ...
    for (nvynum = 0; nvynum < MAXNAVY; nvynum++)
        if (((P_NWSHP + P_NMSHP + P_NGSHP) != 0) && (P_NXLOC == x) && (P_NYLOC == y))
            count++;
...
void prep(int nation, int leader) {
    ...
    for (nation = start; nation < end; nation++)
        if (ntn[nation].active != INACTIVE) {
            ...
            for (armynum = 0; armynum < MAXARM; armynum++) {
                if (leader == TRUE) {
                    if ((P_ATYPE < MINLEADER) || (P_ATYPE >= MINMONSTER) || (P_ASOLD <= 0))
                        continue;
                    i = P_AXLOC;
                    j = P_AYLOC;
                    ...
                    for (x = i - com; x <= i + com; x++)
                        for (y = j - com; y <= j + com; y++)
                            if (ONMAP(x, y))
                                occ[x][y] = safe_int_to_char(nation);
                    ...
                    if ((occ[i][j] == 0) || (occ[i][j] == nation))
                        ...
                        occ[i][j] = safe_int_to_char(NTOTAL);
                }
            }
            if (leader == FALSE)
                for (nvynum = 0; nvynum < MAXNAVY; nvynum++) {
...
void deplete(int nation) {
    ...
    for (armynum = 0; armynum < MAXARM; armynum++)
        if (P_ASOLD > 0) {
            ...
            if (P_ATYPE < MINLEADER && (rand() % 100 < PDEPLETE || P_ATYPE == A_MERCENARY)) {
...
void destroy(int cntry) {
    ...
    if (!ismonst(nptr->active)) {
        ...
        if (cntry != sct[nptr->capx][nptr->capy].owner) {
            ...
            fprintf(fnews, "(they owned their capitol)\n");
    }
 ...
 * @last_documented: 2025-09-19
 */
void updmove(int race, int cntry) {
...
void spreadsheet(int nation) {
    ...
    spread.revothr = spread.revfood = spread.revjewels = spread.revmetal = spread.revcap =
        spread.revcity = 0L;
        ...
        for (y = 0; y < MAPY; y++) {
            ...
            /*harvest food*/
                ...
                if (sptr->people > TOMANYPEOPLE) {
                    ...
                    product +=
                        (long)tofood(sptr, sptr->owner) * (sptr->people - TOMANYPEOPLE) / 2L;
                } else
            ...
            } else if ((sptr->designation == DCITY) || (sptr->designation == DCAPITOL)) {
                cap_pop = sptr->people;
    ...
    spread.gold += spread.revfood + spread.revjewels + spread.revmetal + spread.revcity
                   + spread.revcap + spread.revothr;
}
            ...
            if (now - fst.st_mtime < TIME_DEAD) {
                if (to >= 0 && to < NTOTAL) {
                    snprintf(line, sizeof(line),
                             "Nation %s is reading their mail... try again later.",
                             ntn[to].name);
                    errormsg(line);
                }
                return (-1);
 ...
 */
int markok(int mark, int prtflag /* if true printf reason */
) {
    register int i;
    char temp[LINELTH];
